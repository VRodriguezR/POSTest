                                ...
                                <table class="table table-bordered stripe" id="dataTable" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        @foreach ($marcas as $marca)
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="btn-group" role="group" aria-label="Acciones">
                                                        <a href="{{ route('marcas.edit', $marca->id) }}" class="btn btn-primary">
                                                            <i class="fas fa-edit"></i>
                                                        </a>
                                                        @if ($marca->caracteristica->estado == 1)
                                                            <button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal-{{$marca->id}}">
                                                                <i class="fas fa-trash"></i>
                                                            </button>
                                                        @else
                                                            <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#deleteModal-{{$marca->id}}">
                                            ...
                                            <div class="modal fade" id="deleteModal-{{$marca->id}}" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
                                            <div class="modal-dialog">
                                                ...
                                                <div class="modal-body">
                                                    {{ $marca->caracteristica->estado == 1 ? '¿Estas seguro de eliminar esta marca?' : '¿Estas seguro de activar esta marca nuevamente?'}}
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                                                    <form method="POST" action="{{ route('marcas.destroy', ['marca'=>$marca->id]) }}">
                                                        @method('DELETE')
                                                        @csrf
